...
class HealthCheck:
        ...
        try:
            ...
            if not workers:
                ...
                try:
                    ...
                    logger.error(
                        "No celery workers found and RabbitMQ is not reachable"
                    )
                    return False
                finally:
                    conn.close()
    ...
    async def check_all_services(self, force_cache: bool = True) -> Dict[str, Any]:
    ...
    async def wait_for_services(self, timeout: float = 30.0) -> bool:
        try:
            ...
            while (datetime.now() - start_time) < timedelta(seconds=timeout):
                status = await self.check_all_services(force_cache=False)
